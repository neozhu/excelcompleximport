...
var calc_navbar_height = function () {
    ...
    initApp = (function (app) {
        ...
        app.setLanguage = function () {
            ...
            if (lang && css && culture && $('#dropdownlang').html()) {
        ...
        app.SmartActions = function () {
            ...
            var smartActions = {
                ...
                setLanguage: function ($this) {
                    const lang = $this.data("culture");
                    const flag = $this.data("flag");
                    const text = $this.text();
                    localStorage.setItem('lang-text', text);
                    localStorage.setItem('lang-css', flag);
                    localStorage.setItem('lang-culture', lang);
                    $.get('/Account/SetCulture?lang=' + lang).then(res => {
                        if (res.success) {
                            location.reload();
                        }
                    });
                ...
                userLogout: function ($this) {
                    ...
                    $.SmartMessageBox({
                        title:
                            "<i class='fa fa-sign-out txt-color-orangeDark'></i> 注销 <span class='txt-color-orangeDark'><strong>" +
                            $("#show-shortcut").text() +
                            "</strong></span> ?",
                        content: $this.data("logout-msg") ||
                            "注销后，您可以通过关闭此打开的浏览器来进一步提高安全性。",
                        buttons: "[否][是]"
        ...
        app.domReadyMisc = function () {
            ...
            $("#activity").click(function (e) {
                ...
                $('#lastupdatetime').text('最后更新:  ' + moment().format('h:mm a'));
                //clear memory reference
                $this = null;
                theUrlVal = null;
            ...
            $('input[name="activity"]').change(function () {
                var $this = $(this);
                const notifygroup = $this.attr("notifygroup");
                const curruser = $('#currentuser').val();
                const url = `/Notifications/GetNotifyData?userName=${curruser}&notifygroup=${notifygroup}`;
                $.get(url).done(res => {
                    if (res.data) {
            ...
            $("#btn-loading").on("click",
                function () {
                    ...
                    btn.button("loading");
                    const curruser = $('#currentuser').val();
                    const url = `/Notifications/SetAllRead?userName=${curruser}`;
                    $.get(url).then(res => {
                        btn.button("reset");
                        $('span[name="activity.footer"]').find("span").text('最后更新:  ' + moment().format('h:mm a'));
                        $("#activity").next(".ajax-dropdown").fadeOut(150);
