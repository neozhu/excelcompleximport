...
$(function () {
  ...
  registerNotifyClientMethods(notifyhub);
  // Start Hub
  if ($.connection.hub && $.connection.hub.state === $.signalR.connectionState.disconnected) {
    $.connection.hub.start().done(function (p) {
      registerEvents(chatHub);
      registerNotifyEvent(notifyhub);
      //console.log('start chatHub', p);
    })
      .fail(function () {
        console.error("Could not Connect!");
      });
  } else {
    //console.log("chatHub already Connect!");
  }
  ...
  $('#btnSendMsg').click(function () {
    ...
    if (msg.length > 0) {
      var userName = $('#currentuser').val();
        var fullname = $("#show-shortcut").text();
      var date = GetCurrentDateTime(new Date());
      var avatar = $('#show-shortcut').find('img').attr("src");
      chatHub.server.sendMessageToAll(userName, fullname, msg, date, avatar);
      $("#textarea-expand").val('');
...
function registerClientMethods(chatHub) {
  ...
  chatHub.client.onConnected = function (id, userName, fullName, allUsers, messages, times) {
    //console.log('onConnected', id, userName);
    $('#hdId').val(id);
    //$('#hdfullname').val(fullName);
    // Add All Users
    for (i = 0; i < allUsers.length; i++) {
      if ($('.chat-users').length) {
        AddUser(chatHub, allUsers[i].ConnectionId, allUsers[i].UserName, allUsers[i].FullName, allUsers[i].UserImage, allUsers[i].LoginTime);
      }
    ...
    for (i = 0; i < messages.length; i++) {
      if ($('#chat-body').length) {
        AddMessage(messages[i].UserName, messages[i].FullName, messages[i].Message, messages[i].Time, messages[i].UserImage);
      }
    }
  };

  // On New User Connected
  chatHub.client.onNewUserConnected = function (id, userName, fullName, UserImage, loginDate) {
    if ($('.chat-users').length) {
      AddUser(chatHub, id, userName, fullName, UserImage, loginDate);
    }
  };
  ...
  chatHub.client.messageReceived = function (userName, fullName, message, time, userimg) {

    AddMessage(userName, fullName, message, time, userimg);
...
function GetCurrentDateTime(now) {
  var localdate = moment(now).format("HH:mm");
  return localdate;
}

function AddUser(chatHub, id, name, fullname, UserImage, date) {
...
function AddMessage(userName, fullName, message, time, userimg) {
  ...
  var divChat = '<li class="message" >' +
    ...
    time +
    '</time>' +
    '<a href="javascript:void(0);" class="username">' + fullName + '</a > ' +
    message +
    '</div>' +
    '</li>';
