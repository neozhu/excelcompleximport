...
namespace WebApp
{
  ...
  public class NPOIHelper
  {
    ...
    public static Task<MemoryStream> ExportExcelAsync<T>(string name, List<T> list, Models.ExpColumnOpts[] colopts) => Task.Run(() => {
      ...
      for (var i = 0; i < PropertyInfos.Length; i++)
      {
        ...
        if (colopts.Where(n=> n.FieldName==fieldname && n.IgnoredColumn==true).Any())
        {
          continue;
        }
