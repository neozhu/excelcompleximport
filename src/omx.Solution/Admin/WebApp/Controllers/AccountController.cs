...
namespace WebApp.Controllers
{
  ...
  public class AccountController : Controller
  {
    ...
    public ApplicationDbContext appdbcontext=>HttpContext.GetOwinContext().Get<ApplicationDbContext>();
      ...
      if (user != null)
      {
        switch (await this.SignInManager.PasswordSignInAsync(user.UserName, viewModel.Password, viewModel.RememberMe,shouldLockout: true))
        {
          ...
          default:
            ...
            this.logger.Info($"{user.UserName}:用户名或密码不准确");
            return this.View(viewModel);
        }
      }
    ...
    public ActionResult Register()
    {
      // We do not want to use any existing identity information
      this.EnsureLoggedOut();
      var list = this._companyService.Queryable().Select(x => new ListItem() { Value = x.Id.ToString(), Text = x.Name });

      this.ViewBag.companylist = list;
      var model = new AccountRegistrationModel
      {
        TenantId = Convert.ToInt32(list.FirstOrDefault().Value),
        TenantName = list.FirstOrDefault().Text
      };
    ...
    public async Task<ActionResult> Register(AccountRegistrationModel viewModel)
    {
      ...
      try
      {
        ...
        var user = new ApplicationUser
        {
          ...
          TenantCode = viewModel.TenantCode,
          ...
          AccountType = 0,
          Avatars = viewModel.Gender == 2 ? "femal1" : "male1",
